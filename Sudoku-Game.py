...
class Grid:
    ...
    def update_model(self):
        """Updates the model by sending to solver.
        """
    ...
    def sketch(self, val: int):
        """Sets the temporary number (unconfirmed one) on the cube.

        Args:
            val (int): Number to be sketched on the cube.
        """
        row, col = self.selected
        self.cubes[row][col].set_temp(val)

    def draw(self, win: pygame.display):
        """Draws the gridlines and cubes.
    ...
    def select(self, row: int, col: int):
        """Selects the cube that was clicked.

        Args:
            row (int): Row of the cube that was clicked.
            col (int): Column of the cube that was clicked.
        """
        # Reset all other
        for i in range(self.rows):
            for j in range(self.cols):
    ...
    def clear(self):
        """Clears the unconfirmed number that was typed in the cube.
        """
        row, col = self.selected
        if self.cubes[row][col].value == 0:
            self.cubes[row][col].set_temp(0)

    def click(self, pos: tuple) -> tuple:
        """Finds the position of the cube (space) that we clicked on.

        Args:
            pos (tuple): [description]

        Returns:
            tuple: Position (row, column) of the cube that was clicked.
        """
    ...
    def is_finished(self) -> bool:
        """Checks for empty cubes.

        Returns:
            bool: True if there's no empty cubes, otherwise False.
        """
        for i in range(self.rows):
            for j in range(self.cols):
...
class Cube:
    ...
    def draw(self, win: pygame.display):
        """Draws time, strikes and cubes on the main surface window.
    ...
    def set(self, val: int):
        """Sets the value to given number.

        Args:
            val (int): Number to be set.
        """
        self.value = val
    ...
    def set_temp(self, val: int):
        ...
        Args:
            val (int): Number to be set temporarily.
        """
        self.temp = val
...
def main():
    win = pygame.display.set_mode((540, 600))  # main surface/window
